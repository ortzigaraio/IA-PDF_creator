...
def generar_excel_individual(
    ...
    with temp_png(radar_bytes(dim_display, obj_display, "#" + s["cp"], "#" + s["ca"]), "radar_") as p:
        ws.add_image(OxlImage(p), "H2")

    with temp_png(barras_comparativas_bytes(labels, vals_u, vals_o, "#" + s["cp"], "#" + s["ca"]), "bar_") as p:
        ws.add_image(OxlImage(p), "A23")

    with temp_png(scatter_prioridades_bytes(labels, vals_u, vals_o, "#" + s["ca"]), "scatter_") as p:
        ws.add_image(OxlImage(p), "H23")

    with temp_png(coxcomb_bytes(labels, vals_u), "cox_") as p:
        ws.add_image(OxlImage(p), "A42")
    ...
    with temp_png(gaps_bytes(labels, gaps_vals), "gap_") as p:
        ws_gap.add_image(OxlImage(p), "F1")
