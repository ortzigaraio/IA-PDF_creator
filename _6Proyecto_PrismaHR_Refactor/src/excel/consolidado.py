...
def generar_excel_consolidado_equipo(equipo: List[ItemEquipo], empresa_id: str):
    ...
    with ExitStack() as stack:
        ...
        p_rank = stack.enter_context(temp_png(ranking_equipo_bytes(
                nombres, compat_vals, "#" + s["cp"], "#" + s["ca"], "#dc2626", empresa_info.nombre
        ), "rank_"))
        ws_rank.add_image(OxlImage(p_rank), f"A{3 + len(ordenados) + 3}")
        ...
        p_heat = stack.enter_context(temp_png(heatmap_equipo_bytes(
                heat_nombres, [DIMENSIONES_LABELS[k] for k in DIMENSIONES], heat_mat
        ), "heat_"))
        ws_heat.add_image(OxlImage(p_heat), f"A{4 + len(heat_nombres) + 3}")
